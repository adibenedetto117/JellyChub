...
import { DesktopMediaRow } from './DesktopMediaRow';
import { DesktopContinueWatchingRow } from './DesktopContinueWatchingRow';
...
export function DesktopHomeScreen() {
  ...
  const { data: resumeItems, refetch: refetchResume, isLoading: isLoadingResume } = useQuery({
    ...
    queryFn: () => getResumeItems(userId, 16),
    enabled: !!userId,
    staleTime: 1000 * 30,
  ...
  const { data: allFavorites, refetch: refetchFavorites } = useQuery({
    ...
    queryFn: () => getFavorites(userId, ['Movie', 'Series'], 32),
    enabled: !!userId,
    staleTime: 1000 * 60 * 5,
  });
  ...
  const { favoriteMovies, favoriteSeries } = useMemo(() => {
    ...
    return {
      favoriteMovies: items.filter((item) => item.Type === 'Movie').slice(0, 16),
      favoriteSeries: items.filter((item) => item.Type === 'Series').slice(0, 16),
    };
  }, [allFavorites]);
