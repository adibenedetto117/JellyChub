...
import { getFavoriteSongs, getImageUrl } from '@/api';
...
export default function FavoritesScreen() {
  ...
  const handleTrackPress = (track: BaseItem, index: number) => {
  ...
  const renderTrack = ({ item, index }: { item: BaseItem; index: number }) => {
    ...
    const imageUrl = imageTag
      ...
      : null;
    const artistName = (item as any).Artists?.[0] || (item as any).AlbumArtist || '';
            ...
            tracks.length > 0 ? (
              <View style={styles.header}>
                <View style={[styles.headerIcon, { backgroundColor: accentColor + '20' }]}>
                  <Ionicons name="heart" size={36} color={accentColor} />
                </View>
